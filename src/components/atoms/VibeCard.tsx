...
export default function VibeCard({ emoji, label, selected, onSelect }: VibeCardProps) {
  return (
    <button
      type="button"
      onClick={onSelect}
    ...
    >
      ...
      <span className={`${selected ? 'text-primary' : 'text-main-600'} text-base font-normal font-inter`}>{label}</span>
    </button>
  )
}
