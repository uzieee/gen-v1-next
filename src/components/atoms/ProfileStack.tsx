...
const ProfileCardStack = ({ profiles }: { profiles: Profile[] }) => {
            ...
            {profiles.map((profile, index) => {
                ...
                return (
                    <div
                        ...
                        onClick={isActive ? () => setCurrentIndex((prev) => (prev + 1) % profiles.length) : () => setCurrentIndex(index)}
                    >
                                ...
                                <div className="text-main">
                                    ...
                                    {profile.status && (
                                        <div className="flex items-center gap-2 bg-main/20 backdrop-blur-sm rounded-full px-3 py-1.5 w-fit">
                                            ...
                                            <span className="text-main text-sm font-medium">{profile.status}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
